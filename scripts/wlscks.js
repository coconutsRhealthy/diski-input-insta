...
function updateDiscountsWlscks() {
    return new Promise((resolve, reject) => {
        fs.readFile(discountsPath, 'utf8', (err, data) => {
            ...
            if (group.length > 0) arrays.push(group);

            fs.readFile(wlscksPath, 'utf8', (err, wlscksData) => {
                if (err) return reject(err);

                const wlscksLines = wlscksData
                    .split('\n')
...
module.exports = updateDiscountsWlscks;

if (require.main === module) {
    updateDiscountsWlscks().catch(console.error);
}
